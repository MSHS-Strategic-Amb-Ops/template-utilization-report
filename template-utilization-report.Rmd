...
access_raw <- access_raw_tbl %>%
  ...
  mutate(Appt.DateYear = TO_DATE(CONTACT_DATE),
         Appt.Canceled.DateYear = TO_DATE(APPT_CANC_DATE)) %>%
  ...
  dplyr::select(PAT_ENC_CSN_ID, MRN, DEPARTMENT_NAME, DEPARTMENT_ID, DEPT_SPECIALTY_NAME, SITE, PROV_NAME_WID, PROV_ID, NPI, PRC_NAME, 
                ...
                Appt.Year, Appt.Month, Appt.DateYear, Appt.Canceled.DateYear) %>%
...
```{r Calculate Available Hours, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r Calculate Scheduled Hours, echo = FALSE, warning = FALSE, message = FALSE}

# Process Scheduled and Arrived Hours ------------------------------------------
scheduled_hours <- access_raw_filtered %>%
  filter(Appt.Status %in% c("Arrived", "No Show", "SAmeDay Rescheduled", "SameDay Canceled", "SameDay Bumped")) %>%
  ungroup() %>%
...
```{r Calculate Arrived Hours, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r Calculate Unavailable Hours, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r Calculate On Hold Hours, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r Calculate Private Hours, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r Calculate Overbook Hours, echo = FALSE, warning = FALSE, message = FALSE}
...
<!--   groupBy = c("Appt.MonthYear", "SITE", "DEPT_SPECIALTY_NAME", "DEPARTMENT_NAME", "PROV_NAME"), -->
...
<!--     colGroup(name = "",  -->
<!--              columns = c("Appt.MonthYear","SITE","DEPT_SPECIALTY_NAME","DEPARTMENT_NAME","PROV_NAME"), sticky = "left",  -->
