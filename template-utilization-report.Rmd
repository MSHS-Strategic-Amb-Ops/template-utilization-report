...
# slot_view_tbl <- tbl(conn3, in_schema("OAO_DEVELOPMENT", "AMBULATORY_SLOT_TABLE"))


# poolcon_upt <- dbConnect(odbc(), "OAO Cloud DB Staging", timeout = 15)
# slot_table_query <- glue("SELECT * FROM AMBULATORY_SLOT_TABLE;")
# slot_table_result <- dbGetQuery(poolcon_upt, slot_table_query)
# slot_view_tbl <- slot_table_query
...
slot_util_month <- slot_view_tbl %>%
  ...
  group_by(CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, PROV_ID, PROVIDER, SLOT_MONTH_YEAR, APPT_WEEK) %>%
  ...
  group_by(CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, PROV_ID, PROVIDER, SLOT_MONTH_YEAR) %>%
...
slot_util_summary <- slot_util_summary %>%
  dplyr::select(CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, PROVIDER, PROV_TYPE, SLOT_MONTH_YEAR,
...
```{r Utilization by Month Table Output Provider Only, echo = FALSE, warning = FALSE, message = FALSE}
...
htmltools::browsable(
  tagList(
    tags$button(
      ...
      onclick = "Reactable.downloadDataCSV('template-util-table', 'Template Utilization (Physicians)')"
    ),
...
reactable(
  slot_util_summary %>%
    ...
    mutate(MSHS = "MSHS") %>%
    arrange(CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, PROVIDER) %>%
    dplyr::select(MSHS, CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, PROVIDER, PROV_TYPE, SLOT_MONTH_YEAR,
                avail_hours, avail_hours_week, sched_hours, arrived_hours, 
                sched_util, prov_util,
                avail_hours_day_MON, avail_hours_day_TUE, avail_hours_day_WED, avail_hours_day_THU, avail_hours_day_FRI,
                avail_hours_day_SAT,
