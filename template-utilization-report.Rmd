...
end_date <- format(as.Date(Sys.Date()+14), "%Y-%m-%d") 
start_date <- paste0(format(as.Date(end_date) %m-% months(3), "%Y-%m"),"-01")

start_date_epic <- paste0(format(as.Date("2024-05-01"), "%Y-%m"),"-01")
...
clarity_ser_spec_tbl <- tbl(conn2, "CLARITY_SER_SPEC")
zc_specialty_tbl <- tbl(conn2, "ZC_SPECIALTY")

slot_view_tbl <- tbl(conn3, in_schema("OAO_PRODUCTION", "AMBULATORY_SLOT_TABLE"))

slot_view <- slot_view_tbl %>%
  head(10) %>%
  collect()
...
slot_util_date <- slot_view_tbl %>%
  ...
  group_by(CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, DEPARTMENT_ID, PROV_ID, PROVIDER, SLOT_DATE, APPT_WEEK) %>%
  ...
  group_by(CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, DEPARTMENT_ID, PROV_ID, PROVIDER, SLOT_DATE) %>%
  summarise(avail_hours = sum(avail_hours, na.rm = T),
            avail_hours_week = mean(avail_hours, na.rm = T),
            sched_hours = sum(sched_hours, na.rm = T),
...
slot_util_month <- slot_view_tbl %>%
  ...
  filter(SLOT_DATE >= TO_DATE(start_date, "YYYY-MM-DD HH24:MI:SS")) %>%
  filter(SLOT_DATE <= TO_DATE(end_date, "YYYY-MM-DD HH24:MI:SS")) %>%
  group_by(CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, DEPARTMENT_ID, PROV_ID, PROVIDER, SLOT_MONTH_YEAR, APPT_WEEK) %>%
  summarise(avail_hours = sum(AVAILABLE_MINS, na.rm = T)/60,
            sched_hours = sum(SCHEDULED_MINS, na.rm = T)/60,
            arrived_hours = sum(ARRIVED_MINS, na.rm = T)/60) %>%
  group_by(CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, DEPARTMENT_ID, PROV_ID, PROVIDER, SLOT_MONTH_YEAR) %>%
  summarise(avail_hours = sum(avail_hours, na.rm = T),
            avail_hours_week = mean(avail_hours, na.rm = T),
            sched_hours = sum(sched_hours, na.rm = T),
            arrived_hours = sum(arrived_hours)) %>%
...
slot_util_month <- slot_util_month %>%
    left_join(clarity_ser_tbl %>% 
              dplyr::select(PROV_ID, PROV_TYPE),
            copy = TRUE)
  # left_join(clarity_ser_spec_tbl %>%
  #             dplyr::select(PROV_ID, SPECIALTY_C),
  #           copy = TRUE) 
  # left_join(zc_specialty_tbl %>%
  #             dplyr::select(SPECIALTY_C, NAME),
  #           copy = TRUE) %>%
  # rename("PROV_SPECIALTY" = NAME)
...
slot_util_day <- slot_view_tbl %>%
  ...
  filter(SLOT_DATE >= TO_DATE(start_date, "YYYY-MM-DD HH24:MI:SS")) %>%
  filter(SLOT_DATE <= TO_DATE(end_date, "YYYY-MM-DD HH24:MI:SS")) %>%
  group_by(CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, PROVIDER, SLOT_MONTH_YEAR, APPT_DAY) %>%
  summarise(avail_hours_day = sum(AVAILABLE_MINS, na.rm = T)/60,
            sched_hours_day = sum(SCHEDULED_MINS, na.rm = T)/60,
            arrived_hours_day = sum(ARRIVED_MINS, na.rm = T)/60) %>%
  mutate(sched_util_day = sched_hours_day/avail_hours_day,
            prov_util_day = arrived_hours_day/avail_hours_day) %>%
...
slot_util_summary <- slot_util_summary %>%
  dplyr::select(CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, DEPARTMENT_ID, PROVIDER, PROV_TYPE, SLOT_MONTH_YEAR,
...
*Data shown for pre-Cerner to EPIC go-live should be disregarded: MSH-Amb Care (EPIC go-live in May 2024)*<br/>
*Excludes Non-Epic Scheduling Sites: NYEE*<br/>
...
### TEMPLATE UTILIZATION (Physicians Only)
#### Includes Physicians 
```{r Utilization by Month Table Output Physicians Only, echo = FALSE, warning = FALSE, message = FALSE}
...
htmltools::browsable(
  tagList(
    tags$button(
      ...
      onclick = "Reactable.downloadDataCSV('template-util-physicians-table', 'Template Utilization (Physicians)')"
    ),
...
#       tags$option("None", value = c("CAMPUS","CAMPUS_SPECIALTY","DEPARTMENT","PROVIDER")),
...
reactable(
  slot_util_summary %>%
    filter(CAMPUS %!in% c("NYEE", "OTHER", "MS NOW", "MSHP")) %>%
    filter(!(CAMPUS %in% c("MSH- AMBULATORY CARE") & SLOT_MONTH_YEAR %in% c("2024-03","2024-04"))) %>%
    filter(!is.na(CAMPUS)) %>%
    filter(PROV_TYPE %in% c("Physician")) %>%
    mutate(MSHS = "MSHS") %>%
    arrange(CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, PROVIDER, SLOT_MONTH_YEAR) %>%
    dplyr::select(MSHS, CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, DEPARTMENT_ID, PROVIDER, PROV_TYPE, SLOT_MONTH_YEAR,
                avail_hours, avail_hours_week, sched_hours, arrived_hours, 
                sched_util, prov_util,
                avail_hours_day_MON, avail_hours_day_TUE, avail_hours_day_WED, avail_hours_day_THU, avail_hours_day_FRI,
                avail_hours_day_SAT,
  ...
  elementId = "template-util-physicians-table",
  ...
  groupBy = c("MSHS", "CAMPUS", "CAMPUS_SPECIALTY", "DEPARTMENT", "PROVIDER"),
  ...
  columnGroups = list(
    ...
    colGroup(name = "", 
             columns = c("MSHS", "CAMPUS", "CAMPUS_SPECIALTY", "DEPARTMENT", "PROVIDER", "PROV_TYPE", "SLOT_MONTH_YEAR"), sticky = "left", 
  ...
  columns = list(
    ...
    sched_util = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return totalavail_hours > 0 ? (totalsched_hours) / totalavail_hours   : '-'
      }"),
    ...
    prov_util = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return totalavail_hours > 0 ? (totalarrived_hours) / totalavail_hours  : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    sched_util_day_MON = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return totalavail_hours_day_MON > 0 ? (totalsched_hours_day_MON) / totalavail_hours_day_MON : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    sched_util_day_TUE = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return totalavail_hours_day_TUE > 0 ? (totalsched_hours_day_TUE) / totalavail_hours_day_TUE : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    sched_util_day_WED = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return totalavail_hours_day_WED > 0 ? (totalsched_hours_day_WED) / totalavail_hours_day_WED : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    sched_util_day_THU = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return totalavail_hours_day_THU > 0 ? (totalsched_hours_day_THU) / totalavail_hours_day_THU : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    sched_util_day_FRI = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return totalavail_hours_day_FRI > 0 ? (totalsched_hours_day_FRI) / totalavail_hours_day_FRI : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    sched_util_day_SAT = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return totalavail_hours_day_SAT > 0 ? (totalsched_hours_day_SAT) / totalavail_hours_day_SAT : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    prov_util_day_MON = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return totalavail_hours_day_MON > 0 ? (totalarrived_hours_day_MON) / totalavail_hours_day_MON : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    prov_util_day_TUE = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return totalavail_hours_day_TUE > 0 ? (totalarrived_hours_day_TUE) / totalavail_hours_day_TUE : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    prov_util_day_WED = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return totalavail_hours_day_WED > 0 ? (totalarrived_hours_day_WED) / totalavail_hours_day_WED : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    prov_util_day_THU = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return totalavail_hours_day_THU > 0 ? (totalarrived_hours_day_THU) / totalavail_hours_day_THU : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    prov_util_day_FRI = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return totalavail_hours_day_FRI > 0 ? (totalarrived_hours_day_FRI) / totalavail_hours_day_FRI : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    prov_util_day_SAT = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return totalavail_hours_day_SAT > 0 ? (totalarrived_hours_day_SAT) / totalavail_hours_day_SAT : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
...
### TEMPLATE UTILIZATION (Providers)
#### Includes Physician, Fellow, Resident, Dentist, Physician Assistant, and Nurse Practitioner
```{r Utilization by Month Table Output Providers Only, echo = FALSE, warning = FALSE, message = FALSE}
...
htmltools::browsable(
  tagList(
    tags$button(
      tagList(fontawesome::fa("download"), "Download Table"),
      onclick = "Reactable.downloadDataCSV('template-util-table', 'Template Utilization (Providers)')"
    ),
...
reactable(
  slot_util_summary %>%
    ...
    filter(!(CAMPUS %in% c("MSH- AMBULATORY CARE") & SLOT_MONTH_YEAR %in% c("2024-03","2024-04"))) %>%
    filter(!is.na(CAMPUS)) %>%
    filter(PROV_TYPE %in% c("Physician", "Resident", "Fellow", "Dentist", "Physician Assistant", "Nurse Practitioner")) %>%
    mutate(MSHS = "MSHS") %>%
    arrange(CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, PROVIDER, SLOT_MONTH_YEAR) %>%
    dplyr::select(MSHS, CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, DEPARTMENT_ID, PROVIDER, PROV_TYPE, SLOT_MONTH_YEAR,
                avail_hours, avail_hours_week, sched_hours, arrived_hours, 
  ...
  columns = list(
    ...
    sched_util_day_MON = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalsched_hours_day_MON) / totalavail_hours_day_MON > 0 ? (totalsched_hours_day_MON) / totalavail_hours_day_MON : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    sched_util_day_TUE = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalsched_hours_day_TUE) / totalavail_hours_day_TUE > 0 ? (totalsched_hours_day_TUE) / totalavail_hours_day_TUE : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    sched_util_day_WED = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalsched_hours_day_WED) / totalavail_hours_day_WED > 0 ? (totalsched_hours_day_WED) / totalavail_hours_day_WED : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    sched_util_day_THU = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalsched_hours_day_THU) / totalavail_hours_day_THU > 0 ? (totalsched_hours_day_THU) / totalavail_hours_day_THU : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    sched_util_day_FRI = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalsched_hours_day_FRI) / totalavail_hours_day_FRI > 0 ? (totalsched_hours_day_FRI) / totalavail_hours_day_FRI : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    sched_util_day_SAT = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalsched_hours_day_SAT) / totalavail_hours_day_SAT > 0 ? (totalsched_hours_day_SAT) / totalavail_hours_day_SAT : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    prov_util_day_MON = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalarrived_hours_day_MON) / totalavail_hours_day_MON > 0 ? (totalarrived_hours_day_MON) / totalavail_hours_day_MON : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    prov_util_day_TUE = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalarrived_hours_day_TUE) / totalavail_hours_day_TUE > 0 ? (totalarrived_hours_day_TUE) / totalavail_hours_day_TUE : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    prov_util_day_WED = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalarrived_hours_day_WED) / totalavail_hours_day_WED > 0 ? (totalarrived_hours_day_WED) / totalavail_hours_day_WED : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    prov_util_day_THU = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalarrived_hours_day_THU) / totalavail_hours_day_THU > 0 ? (totalarrived_hours_day_THU) / totalavail_hours_day_THU : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    prov_util_day_FRI = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalarrived_hours_day_FRI) / totalavail_hours_day_FRI > 0 ? (totalarrived_hours_day_FRI) / totalavail_hours_day_FRI : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    prov_util_day_SAT = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalarrived_hours_day_SAT) / totalavail_hours_day_SAT > 0 ? (totalarrived_hours_day_SAT) / totalavail_hours_day_SAT : '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
...
htmltools::browsable(
  tagList(
    tags$button(
      tagList(fontawesome::fa("download"), "Download Table"),
      onclick = "Reactable.downloadDataCSV('template-util-all-table', 'Template Utilization (Other)')"
    ),
...
reactable(
  ...
  elementId = "template-util-all-table",
  # elementId = "cars-grouping-table",
  style = list(fontFamily = 'Calibri',
                 fontSize = '14px'),
    defaultColDef = colDef(align = "left",
  ...
  groupBy = c("MSHS", "CAMPUS", "CAMPUS_SPECIALTY", "DEPARTMENT", "PROV_TYPE"),
  ...
  columnGroups = list(
    ...
    #          headerStyle = list(color = "#d80b8c", fontSize = "16px"))
    colGroup(name = "", 
             columns = c("MSHS", "CAMPUS", "CAMPUS_SPECIALTY", "DEPARTMENT", "PROV_TYPE", "PROVIDER", "SLOT_MONTH_YEAR"), sticky = "left", 
             headerStyle = list(fontSize = "16px")),
    colGroup(name = "Available Hours", 
             columns = c("avail_hours","avail_hours_week"),
             headerStyle = list(color = "#7f7f7f", fontSize = "16px")),
