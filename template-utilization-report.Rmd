...
avail_hours <- availability_raw_tbl %>%
  ...
  mutate(no_openings = ifelse(ORG_REG_OPENINGS == 0, "no_opening", "reg_opening")) %>%
  group_by(DEPARTMENT_NAME, DEPARTMENT_ID, PROV_NM_WID, PROV_ID, Appt.Year, Appt.Month, Appt.DateYear, SLOT_BEGIN_TIME, SLOT_LENGTH, ORG_REG_OPENINGS) %>%
  summarise(avail_min = n()) %>%
  mutate(relevant_slot = "Yes")


access_raw_filtered <- access_raw %>%
  left_join(avail_hours %>% 
              dplyr::select(DEPARTMENT_ID,PROV_ID,SLOT_BEGIN_TIME,relevant_slot), 
